...
- **624 — Expiración de ofertas**: un precio `sale` puede incorporar `expires_at`. La política RLS de lectura pública y las consultas de catálogo excluyen automáticamente los precios vencidos; no se actualiza ni borra el historial. La comprobación queda acompañada de una prueba reproducible contra Supabase tras aplicar la migración aprobada. No se introduce un cron prematuro: las operaciones programadas siguen siendo responsabilidad de Fase 9.

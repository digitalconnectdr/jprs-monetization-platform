...
## Actualización posterior al cierre (2026-08-08) — enriquecimiento del catálogo

A pedido explícito del propietario funcional ("las comparaciones son muy sencillas... como podemos hacer esa comparacion mucho mejor... para atraer visitas"), se agregaron dimensiones reales nuevas a los 5 productos ya sembrados — no reabre la fase, mismo criterio que la adición de francés en Fase 3:

- **Nuevas features investigadas y sembradas** (`supabase/seed_p6a_enrich.sql`, todas con `source`+`checked_at`, mayoría `confidence='verified'` vía `WebFetch` directo): `billing_model` (mensual vs. anual, y el descuento real de cada uno) y `marketplace_integrations` (solo HubSpot publica un número exacto — "2,000+ apps" — Freshsales y monday.com tienen marketplace pero ninguna página oficial fetcheada publica un total, así que **no se inventó un número** para esos dos, mismo criterio que descartó Luggage en Fase 6B) para los 3 CRMs; `api_access` y `team_plan` para ChatGPT/Claude.
- **CRM Pricing Comparator** ampliado de 3 a 5 columnas (Billing, Integrations nuevas).
- **Nuevo tipo de bloque `verdicts`** en el renderer de guías (`ComparisonTableBlock`/`ProsConsBlock` ya existían; `VerdictsBlock` es nuevo) — veredictos "mejor para X caso de uso" citando las dimensiones nuevas. Se creó la **versión 2** del artículo pendiente (content_item `70fbd00f-6ce7-4962-83fc-d14b7528cab3`), que sigue en `pending_editorial_review` — el enriquecimiento no cambia el gate de ADR-005, solo hace más rica la versión que espera aprobación.
- **Limitación de verificación**: la versión 2 del artículo se verificó estructuralmente (datos correctos vía query directa, patrón de render idéntico al de `pros_cons` ya probado en producción) pero no se pudo confirmar visualmente en la ruta pública real, porque esa ruta solo renderiza contenido `published` — el mismo gate de ADR-005 que impide la auto-aprobación también impide una vista previa pública sin publicar. No se fuerza el estado para verlo.

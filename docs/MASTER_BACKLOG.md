...
| 607 | P0 | Completar el resto del seed editorial de Software & AI (10-15+ páginas adicionales, 4 categorías restantes: Automation, SEO & Marketing, Website & E-commerce, Productivity) | Editorial PASS | DONE (parcial, 2026-08-14) — **catálogo** completo en las 6 categorías: Automation (Zapier/Make/n8n), SEO & Marketing (Ahrefs/Semrush/Surfer SEO), Website & E-commerce (Shopify/WordPress.com — Squarespace/Wix descartados: precio no verificable, renderizado por JS), Productivity (Notion/Asana/ClickUp). 11 productos nuevos, precio/specs verificados en sus páginas oficiales (agosto 2026), confirmados en `sitemap.xml`. Sigue pendiente el **contenido editorial narrativo** (guías "vs"/"best X" escritas, mismo patrón que 606/616/626) para estas 4 categorías — nuevo backlog 631 |
...
| 630 | P1 | Completar "eero 7 vs Nest Wifi Pro vs Deco BE63" con tabla comparativa y pros/cons por producto (nueva versión, `content_versions.version_number=2`) antes de volver a someterla a revisión editorial | Versión completa en `pending_editorial_review` | DONE (2026-08-14) — v2 creada con 3 bloques pros/cons + veredictos + 3 fuentes citadas, usando specs ya sembradas en Fase 6C (sin necesitar investigación externa nueva). En `pending_editorial_review`, esperando decisión del propietario funcional |
| 631 | P1 | Redactar guías/comparaciones narrativas ("vs"/"best X") para las 4 categorías de Software & AI recién sembradas (Automation, SEO & Marketing, Website & E-commerce, Productivity) — catálogo real ya existe, falta el contenido editorial | Contenido en `pending_editorial_review`, mismo patrón que 606/616/626 | TODO — catálogo completo (backlog 607), contenido narrativo pendiente |
| 632 | P2 | Corregir métrica "Approved, unpublished" en `/admin/content`: contaba TODAS las `content_versions` con `review_state='approved'`, sin excluir las que ya son `current_version_id` de su item (ya publicadas) — hallazgo de la revisión propia post-sesión, no reportado por el usuario | QA PASS | DONE (2026-08-14) — `getContentSummary` ahora calcula `approvedUnpublishedCount` excluyendo versiones que ya son la versión vigente publicada de su item. Verificado en navegador: pasó de mostrar 2 (incorrecto) a 0 (correcto, dado que las 2 versiones aprobadas de 606/616 ya están publicadas) |
| 633 | P2 | Corregir "Sessions" en `/admin/acquisition`: se calculaba como suma de sesiones distintas por site, sobredimensionando el total cada vez que una misma sesión (mismo `session_id` en `localStorage`) visita más de un site — hallazgo de la revisión propia post-sesión, reproducido en vivo navegando 4 sites durante el QA (mostró 20 en Acquisition vs. 12 real en Executive, mismo dataset) | QA PASS | DONE (2026-08-14) — `getAcquisitionSummary` ahora devuelve `totalSessions` calculado con un Set global de `session_id` sobre todas las filas, separado del desglose `sessionsBySite` (que sigue siendo válido tal cual, per-site). UI actualizada con nota aclaratoria de por qué la suma por site puede superar el total. Verificado en navegador: Sessions pasó de 20 a 12, ahora consistente con Executive |
| 634 | P2 | Sistema de rating/estrellas tipo G2/Capterra en fichas de producto — el propietario funcional lo identificó como la pieza estructural que falta para que el sitio se sienta como esos comparadores | N/A | DEFERRED (2026-08-14) — el propietario funcional pidió dejarlo para revisión futura explícitamente, no iniciar ahora. Nota de diseño ya discutida: un promedio de estrellas crowdsourced (como G2/Capterra) requeriría reseñas reales de usuarios verificados, que hoy no existen (sin cuentas públicas, sin tráfico orgánico) — mostrar un promedio sin reseñas reales violaría la regla de "nunca fabricar" (`CONTENT_POLICY.md`). La alternativa viable sin inventar datos sería un "Decidero Score" editorial derivado de specs/precios/pros-cons ya sembrados (mismo concepto que "Quality Score" de `MONETIZATION_POLICY.md`, hoy no visualizado). Gate para retomar: cuando haya tráfico/usuarios reales que justifiquen reseñas genuinas, o cuando el propietario funcional decida avanzar con la variante editorial |

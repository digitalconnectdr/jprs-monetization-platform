...
Config interna por site (`editor`/`admin` gestionan, sin lectura pública — no existe renderer de ads todavía). `monetization_rules.page_type` incluye `auth`/`admin`/`low_value` además de los tipos de `PROJECT_BLUEPRINT.md` §7.1, precisamente para que el `CHECK constraint monetization_rules_no_ads_on_prohibited_pages` pueda impedir `'ads' = any(allowed_layers)` en esos tres — estructuralmente imposible de insertar, no solo una regla documentada (`MONETIZATION_POLICY.md` §4/§9). `allowed_layers` además está restringido a un vocabulario fijo (`CHECK monetization_rules_allowed_layers_vocabulary`: solo `ads`/`affiliate`/`lead`/`sponsor`, exactos en minúscula) — agregado tras F-02 de `docs/audits/P5_AUDIT.md`, la comparación de igualdad de string original era evadible con `'Ads'`/`'ADS'`/`' ads'`; Postgres no permite subqueries dentro de un `CHECK` (`EXISTS`+`unnest` falla con "cannot use subquery in check constraint"), así que restringir el vocabulario de entrada es lo que hace la comparación de igualdad simple confiable de nuevo.
...
- `has_role_in_niche(role_name text, p_niche_id uuid) → boolean`: equivalente a `is_admin_for_niche` pero para cualquier rol, no solo `admin`. **Corrige un bug real encontrado durante el testing de Fase 5** (antes de la auditoría): las policies que resolvían el niche de un vendor/lead_form uniendo `sites` directamente fallaban para `editor`, porque `editor` no tiene ninguna policy de lectura sobre un site en estado `draft` (solo `admin`/`super_admin` vía `sites_admin_select`, o público si el site está `active`). `SECURITY DEFINER`, mismo patrón que `is_admin_for_niche`. **`EXECUTE` revocado de `PUBLIC`** (`20260808050000_fix_p5_audit_findings.sql`, F-01 de `docs/audits/P5_AUDIT.md`) — Postgres otorga `EXECUTE` a `PUBLIC` por defecto al crear una función, y el `GRANT` explícito a `authenticated`/`service_role` de la migración original no revocaba eso, dejándola llamable sin sesión (inofensivo en este caso porque depende de `auth.uid()`, `null` para `anon`, pero corregido por higiene y consistencia con `site_niche_id`).
- `site_niche_id(p_site_id uuid) → uuid`: resuelve el `niche_id` de un site sin depender de que el rol que llama tenga lectura RLS sobre esa fila — usado en los cross-checks de scope de `affiliate_offers`/`lead_routes`/`sponsorship_placements`. `SECURITY DEFINER`. **`EXECUTE` revocado de `PUBLIC`** — a diferencia de `has_role_in_niche`, esta SÍ era explotable sin sesión: cualquier visitante podía llamar `rpc/site_niche_id` con cualquier UUID de site y obtener su `niche_id` real, incluyendo sites `draft` que ninguna policy expone a `anon` (F-01, High, confirmado empíricamente por el auditor contra el proyecto real).
...
- **`REVOKE EXECUTE ... FROM PUBLIC` en toda función `SECURITY DEFINER` nueva, desde el día uno**: PostgreSQL otorga `EXECUTE` a `PUBLIC` por defecto al crear una función — un `GRANT` explícito a `authenticated`/`service_role` no revoca eso, así que sin un `REVOKE` explícito la función queda llamable por `anon` sin ninguna sesión (F-01 de `docs/audits/P5_AUDIT.md`, High, confirmado explotable en producción). Backlog 411 (nuevo): revisión sistemática de `has_role`/`is_admin_for_site`/`is_admin_for_niche`/`import_product_prices` (Fases 2 y 4) para confirmar si comparten este gap — ninguna es explotable de forma independiente hoy según el auditor de Fase 5 (dependen de `auth.uid()` o tienen su propio chequeo interno), pero revocar `PUBLIC` de forma sistemática requiere verificar primero que ninguna policy de lectura pública coexistente en la misma tabla dependa del privilegio implícito.
